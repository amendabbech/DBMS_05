...
>
> ![image alt](https://github.com/amendabbech/DBMS_05/blob/58b8689c435ae7b545d7654ac031bc6beee4007a/1.png)
...
>  tagesgebuehr should not be stored as REAL because floating-point values can produce rounding errors. Example: 0.10 + 0.20 = 0.30000000000000004 A lending fee calculation such as: 0.30 × 3 = 0.899999999 would produce an incorrect monetary result. Therefore, monetary values must be stored using: NUMERIC(p,s) because it stores decimal numbers exactly.
...
> NULL in rueckgabe_datum means that the return date is currently unknown because the book has not yet been returned. NULL is not the same as “zero days.” It represents the absence of a value. SQL uses three-valued logic: TRUE,FALSE and UNKNOWN. If rueckgabe_datum is NULL, comparisons involving this value return UNKNOWN.
...
> The default expression should be: DEFAULT CURRENT_DATE This is preferable because the database automatically inserts the current date when no value is provided. It reduces application errors and ensures consistent data entry.
...
>
> ![image alt](https://github.com/amendabbech/DBMS_05/blob/03d0f1ed084c0c94f33830893b4e4a6f3455a8fe/2.png)
...
>
> - Test A: CHECK constraint failed: tagesgebuehr > 0
...
> A column constraint is insufficient because it can only reference a single column, while the condition compares rueckgabe_datum and ausleihe_datum. Therefore a table-level CHECK is required.
...
> ON DELETE CASCADE would be appropriate between buch and exemplar, because when a book is deleted, all its copies should also be deleted to avoid orphan records.
...
> A UNIQUE column may contain multiple NULL values, because NULL represents an unknown value and SQL treats NULL comparisons as UNKNOWN in three-valued logic, so NULLs are not considered equal.
